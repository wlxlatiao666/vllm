...
class Sequence:
    ...
    def get_output_len(self) -> int:
        # ``new_branch_token_id`` is inserted into a child sequence's prompt
        # by ``ParallelSampleSequenceGroup.add_tree_branches``.  It is exposed
        # again through ``CompletionOutput.tree_ids`` so callers can rebuild
        # the tree path, but it is not an output token generated by this child
        # request.  Counting it here would consume one ``max_tokens`` slot at
        # every tree depth and would also break delta-output offsets.
        return self.data.get_output_len()

    def get_tree_segment_len(self) -> int:
        """Return the visible token length of this tree segment.

        Unlike :meth:`get_output_len`, this includes the forced branch token
        stored in the child prompt.  Tree-specific minimum-segment checks may
        use this value without changing vLLM's core output-length semantics.
        """
        return self.data.get_output_len() + int(
            self.new_branch_token_id is not None)
...
class ParallelSampleSequenceGroup(SequenceGroupBase):
    ...
    def add_tree_branches(self, parent_req_id: str, new_token_ids: list[int], engine, deferred: bool = False):
        ...
        for i, token_id in enumerate(valid_token_ids):
            ...
            params.n = 1
            params.tree_search_params.enable_tree_search = False
            # get_output_len() counts only tokens generated by this request.
            # The forced branch token is already in the child prompt and must
            # not consume a second output-budget slot.
            params.max_tokens = (
                parent_seq_group.sampling_params.max_tokens
                - parent_seq.get_output_len())
            if deferred:
                # Deferred branching drops both t1 and t2 but replaces only t1,
                # so one additional generated token is needed to preserve the
                # original end-to-end response budget.
                params.max_tokens += 1
            seq_group = engine._add_processed_request(
                request_id_i,
